...
```bash
conda create -n dbx_experiment python=3.9
conda activate dbx_experiment
```
...
```
This command will create your personal repository under `/Repos/<username>/dbx_experiment`.
...
- To trigger the release pipeline, get the current version from the `dbx_experiment/__init__.py` file and tag the current code version:
```
